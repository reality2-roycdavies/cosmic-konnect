...
use super::crypto::{
    ...
    SessionCrypto, PUBLIC_KEY_SIZE, SESSION_NONCE_SIZE,
};
use super::message::*;
use super::{CONNECTION_TIMEOUT_SECS, MAX_MESSAGE_SIZE, TCP_PORT};

/// Connection state
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ConnectionState {
    /// Just connected, awaiting identity
    Connected,
    /// Identity exchanged
    Identified,
    /// Pairing in progress
    Pairing,
    /// Connected and encrypted (paired device)
    Encrypted,
    /// Disconnected
    Disconnected,
}
...
pub enum ConnectionEvent {
    /// Connection established with a device
    Connected {
        device_id: String,
        device_name: String,
        address: SocketAddr,
...
pub enum ConnectionCommand {
    ...
    Send(Message),
    /// Request pairing (includes our key pair)
...
impl DeviceConnection {
    ...
    pub async fn accept_pairing(&self) -> Result<(), ConnectionError> {
        self.command_tx
    ...
    pub async fn reject_pairing(&self) -> Result<(), ConnectionError> {
        self.command_tx
...
impl ConnectionManager {
    ...
    pub async fn is_paired(&self, device_id: &str) -> bool {
        self.paired_devices.read().await.contains_key(device_id)
    }
    ...
    pub async fn request_pairing(&self, device_id: &str) -> Result<(), ConnectionError> {
        ...
        // Check if device is in connections map
        let connections = self.connections.read().await;
        let conn_ids: Vec<_> = connections.keys().collect();
        info!("Current connections: {:?}", conn_ids);

        if let Some(conn) = connections.get(device_id) {
            // Generate key pair and send command to connection handler
            let key_pair = KeyPair::generate();
            info!("Sending RequestPairing command to {}", device_id);
            conn.command_tx
...
async fn handle_connection(
    ...
    // Check if already paired - if so, set up encryption
    let mut session_crypto: Option<SessionCrypto> = None;

    {
        let paired = paired_devices.read().await;
        if let Some(pairing_info) = paired.get(&device_id) {
            // Set up session encryption
            if let (Some(our_nonce), Some(peer_nonce)) = (
                identity_msg.session_nonce.as_ref(),
                peer_identity.session_nonce.as_ref(),
            ) {
                session_crypto = Some(SessionCrypto::new(
                    &pairing_info.pairing_key,
                    our_nonce,
                    peer_nonce,
                ));
                info!("Encrypted session established with {}", device_name);
            }
        }
    }
        ...
        tokio::select! {
            ...
            result = read_message(&mut reader) => {
                match result {
                    Ok((message, flags)) => {
                        // Decrypt if needed
                        let message = if flags.encrypted {
                            if let Some(ref crypto) = session_crypto {
                                // TODO: Actually decrypt the message payload
                                message
                            } else {
                                warn!("Received encrypted message but no crypto context");
                                continue;
                            }
                        } else {
                            message
                        ...
                        match &message {
                            Message::PairRequest(req) => {
                                ...
                                if req.public_key.len() == PUBLIC_KEY_SIZE {
                                    ...
                                    // Calculate verification code (we need the shared secret)
                                    // For now, show a placeholder - proper implementation would
                                    // do the key exchange first
                            ...
                            Message::PairResponse(resp) => {
                                info!("Received PairResponse from {} - accepted: {}", device_id, resp.accepted);
                                if resp.accepted {
                                    let has_key_pair = pairing_key_pair.is_some();
                                    let peer_key_len = resp.public_key.as_ref().map(|k| k.len()).unwrap_or(0);
                                    info!("PairResponse: has_key_pair={}, peer_key_len={}", has_key_pair, peer_key_len);
                                    ...
                                    if let (Some(peer_key_bytes), Some(kp)) = (resp.public_key.as_ref(), pairing_key_pair.take()) {
                                        if peer_key_bytes.len() == PUBLIC_KEY_SIZE {
                                            ...
                                            warn!("PairResponse has invalid key size: {}", peer_key_bytes.len());
                                        }
                                    } else {
                                        warn!("Cannot complete pairing: missing key pair or peer key");
                                    }
                                } else {
                                    info!("Pairing rejected by {}: {:?}", device_id, resp.reason);
            ...
            Some(cmd) = command_rx.recv() => {
                match cmd {
                    ConnectionCommand::Send(message) => {
                        let flags = if session_crypto.is_some() {
                            MessageFlags { encrypted: true, ..Default::default() }
                        } else {
                    ...
                    ConnectionCommand::RequestPairing(key_pair) => {
                        ...
                        // Store the key pair for when we receive the response
                        ...
                        info!("Sending PairRequest to {}", device_id);
                        match request.encode(MessageFlags::default()) {
                            Ok(data) => {
                                if let Err(e) = writer.write_all(&data).await {
                            ...
                            Err(e) => {
                                error!("Failed to encode PairRequest: {}", e);
                            }
                        }
                    }
...
async fn read_message<R: AsyncReadExt + Unpin>(
    ...
    let mut header = [0u8; 8];
    debug!("read_message: waiting for header...");
    reader.read_exact(&mut header).await?;
    debug!("read_message: got header: {:02x?}", &header);
    ...
    // First extract the type field, then deserialize the correct struct
    ...
    debug!("read_message: decoded {:?}", message.message_type());
    Ok((message, flags))
}
